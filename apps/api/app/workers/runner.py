...
class TaskRunner:
    ...
    async def _claim_and_execute(self) -> None:
        async with self.session_factory() as session:
            ...
            if not tasks:
                return
            for task in tasks:
                await self._execute_one(session, task)

    async def _execute_one(self, session: AsyncSession, task: Task) -> None:
        """执行单个任务并确认 attempt。"""
        run_token = task.run_token
        attempt_no = task.attempt_count
        queue = TaskQueue(session)
        try:
            # 再次加锁读取（claim 已提交，可能被 reaper 改动）。
            fresh = (
            ...
            try:
                ...
                await self._mark_cancelled(queue, task, run_token)
                ...
                error_code = TaskErrorCode.TEMPORARY_INFRA_ERROR
                await self._handle_failure(
                    queue, task, run_token, attempt_no,
                    error_code, "任务执行超时", retriable=True,
                )
                ...
                await self._handle_failure(
                    queue, task, run_token, attempt_no,
                    error_code, str(exc), retriable=retriable,
                )
            else:
                # handler 成功返回：标记 completed（CAS）。
    ...
    async def _mark_cancelled(self, queue: TaskQueue, task: Task, run_token: uuid.UUID) -> None:
