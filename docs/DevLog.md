...
## 修复启动日志编码与终端实时显示（2026-09-17）

- 确认现有 API、worker、Web 日志均为 UTF-8 文件头与 UTF-16 内容混写：启动脚本使用 `Add-Content -Encoding UTF8`，后台 Windows PowerShell 5.1 却通过 `*>>` 以 UTF-16 追加；隐藏后台进程也未将输出转回启动终端。
- 后台统一使用 UTF-8 `StreamWriter` 按行保存标准输出和错误输出并即时刷新，显式设置 Python UTF-8/无缓冲输出和 PowerShell 原生命令解码；通过 `-EncodedCommand` 传递启动内容，支持中文路径，保留原生命令退出码。失败日志读取同样显式指定 UTF-8。
- 每次启动前将上一轮日志保留为带时间戳的 `.bak`，当前日志重新使用 UTF-8，避免继续混写；旧日志备份不自动转换。启动完成并打印访问地址后，当前终端持续显示带 `[API]`、`[Worker]`、`[Web]` 前缀的日志；`Ctrl+C` 结束查看并释放读句柄，停止应用仍使用 `dev-stop.ps1`。
- 在 `DatasetGen` 环境中用独立的真实 Python/Node 子进程验证中文标准输出/错误输出、严格 UTF-8 解码、长行无折行截断、0/7 退出码、三路日志汇总、到达文件末尾后继续读取新增内容、旧日志完整备份及下一轮日志写入。验证未调用项目数据库或重启现有服务；现有服务需重新运行启动脚本才能使用新写入方式。
- 同步更新根目录与脚本目录 README，说明实时日志、退出查看、单文件查看及 Docker 容器日志命令。本次未增加测试脚本或运行测试套件。

## 启动时自动拉起 Docker Desktop（2026-09-17）

- 调整 `scripts/dev-start-conda.ps1` 的 Docker 启动流程：`docker info` 成功时复用当前引擎；失败时执行 `docker desktop start --timeout 120`，等待 Desktop 启动完成，再次检查引擎后才运行 Compose。自动启动或引擎检查失败时中止，并提示首次初始化、WSL 2/虚拟化、CLI 命令支持或 Docker context 的排查方向。
- 更新根目录及脚本目录 README，说明新机器会自动拉取缺失镜像、创建容器/网络/端口映射/数据卷并初始化数据库和存储桶；Docker Desktop 首次初始化、所选后端的系统设置、Linux 容器模式及项目运行依赖仍需提前准备。
- 验证本机 `docker desktop start --help` 支持 `--timeout`，当前 Docker 引擎为运行中的 Linux 模式；PowerShell 语法检查通过，在内存中替换外部命令验证引擎已就绪、引擎未启动后成功拉起、Desktop 启动失败三个分支。未停止或重启本机 Docker，未执行完整项目启动或测试套件；冷启动仍需在 Docker 已退出的实际场景确认。

## 精简运行脚本（2026-09-17）

- 按用户要求，以 `scripts/` 及其直接关联入口为范围，统一 Windows PowerShell + conda `DatasetGen` 运行方式。删除 21 个 Bash、备用 uv 启动、测试、审计、旧配置迁移、数据重置、导出孤儿清理及 OpenAPI 导出脚本，保留 `dev-start-conda.ps1`、`dev-stop.ps1`、`init_seed.py`、`mineru_local_service.py`、`paddleocr_local_service.py` 五个运行脚本及目录说明。
- 两个本地解析服务脚本仍被后端服务管理器实际调用，因此保留；数据库结构迁移继续由 Alembic 在启动时执行。
- 同步删除直接依赖退役脚本的前端接口生成/检查工具、相关专项测试和 E2E 工作流，共 10 个关联文件；移除对应 npm `api:generate`、`api:check` 命令。前端现有接口类型、业务代码、配置及业务数据保留。
- 更新根目录和脚本目录 README，清理已退役入口及测试说明；启动/停止脚本的提示统一为 API、worker 与 Web。删除项均为无未提交修改的 Git 跟踪文件，可从 Git 历史恢复。
- 验证：历史开发日志之外无退役文件的残留引用；两个 PowerShell 运行脚本语法及 npm 运行命令检查通过；在 `DatasetGen` 环境中以禁用字节码写入的方式成功导入 API、worker、初始化模块，并确认两个解析服务脚本存在。未启动服务、写数据库或运行测试套件。

## 清理临时文件与中间产物（2026-09-17）

- 按用户要求，将 57 个临时文件或目录移入 Windows 回收站，共 1529 个文件、约 218.47 MiB，可从回收站恢复。
- 清理 Next.js 构建产物与 TypeScript 增量缓存、Python 字节码、pytest/ruff 缓存、覆盖率报告、Playwright 快照、历史日志、失效 PID 文件，以及 MinerU/PaddleOCR 的历史解析测试输出和冒烟测试产物。
- 清理前确认 API、Web、worker 已停止，逐项核对目标路径均位于项目内、不含目录链接或 Git 跟踪文件；保留源码、依赖环境、模型权重、解析器配置、原始 PDF、BUG 记录和数据库/对象存储业务数据。
- 清理后核对目标均已移除；本次未修改业务代码，未运行应用测试。下次启动或测试时会按需重新生成缓存。

## 删除 GitHub CI 配置（2026-09-15）

- 按用户要求删除 `.github` 目录；目录中仅有 `workflows/ci.yml`，移除整个 CI 工作流，取消 push 和 PR 自动检查。
- 本次仅删除 CI 配置，检查配置差异及空白错误，不运行业务测试。
...
- 删除对应的五个本地 `fix/review-*` 分支。此前已通过补丁等价检查及 range-diff 核对，业务改动均已整合进 master；差异仅涉及整合上下文与开发日志。
- 删除前再次确认五个工作副本均无未提交改动或未跟踪文件；完成后 Git 仅保留主工作副本与 master 本地分支。
- 本次仅清理 Git 工作副本和分支，未修改业务代码，未运行应用测试。

---

## 用户流程审查修复收口与浏览器验收（2026-09-12）

- 补齐第二批和第三批记录：清洗会话改为租约驱动、串行保存和提交前 flush，空字符串保持明确删除语义；导出沿固定 Chunk→ChunkSet→清洗版本→ParseJob 来源链构建 manifest，并阻止不兼容内容生成空训练样本。
- 补齐第六批：批量生成支持合法分页与跨页选择，终态批次可新建生成；PowerShell 质量脚本以退出码判断原生命令结果，Vitest 配置改为 `.mts`，消除模块加载警告。
- 修复真实浏览器验收发现的 `SectionResponse.source_pages` 合同：解析分段返回页码数组，API/Chunk/Evidence 兼容数组和历史对象，避免清洗章节列表 500。
- 修复隔离测试基础设施：MinIO 初始化容器接收测试环境变量，长期服务与一次性初始化分开等待，`test-infra` 可稳定返回成功。
- 新增可重复浏览器验收脚本和本地 OpenAI-compatible stub；使用隔离 `datasetgen_e2e`、MinIO 前缀和端口，真实登录后完成两份 PDF、双解析、清洗审核、双分块、生成、跨文档证据、Dataset 导出及深度校验。未调用真实付费模型或解析服务。
- 最终验证：后端 507 项 pytest 通过、覆盖率 67.20%；前端 142 项 Vitest、ESLint、TypeScript、OpenAPI 契约检查和生产构建通过；r04 迁移空库往返通过。

---

## 工作流修复第一批：认证与项目权限（2026-09-12）

- 对应 R01/R02/R20：认证初始化等待用户资料，退出、切换账号和卸载使旧请求失效；登录资料完整后才写入令牌，登录后恢复安全的站内地址。
- 新增项目 access 接口，详情和成员列表补齐成员鉴权；数据集、评测集、知识审批和清洗分派使用统一有效项目角色 hook。
- 新增/加强认证时序、跨账号晚到响应、角色反转、权限撤销、项目切换、站内回跳与跨项目读取回归，更新 OpenAPI 与 TypeScript 契约。
- 验证：前端全量 106 项（回跳控制字符用例随后追加）通过，ESLint、TypeScript、后端 Ruff 通过；新增项目 API 集成测试 2 项通过。最终整合后再次执行全量门禁与浏览器验收。
- 保留本轮审查报告，修复状态单独记录；未清空开发业务数据。

---

## 用户流程审查修复第五批：解析配置、设置与试跑（2026-09-12）

- 对应 R03、R04、R15、R21 前端：解析器 seed、CRUD、快照冻结共用参数白名单；本地 MinerU 无 HTTP endpoint，服务型解析器仅在 registry 已注册时生成可用配置。
- 设置页改为切分、导出和任务策略三类准确 DTO 表单。编辑仅 PATCH 改动字段，保留零值，JSON 参数可回读和显式清空；任务策略按六种可执行类型创建和设默认。
- 模板试跑复用正式 renderer、模型快照和 LLMClient，修复重复 `/v1`、未设置采样参数和 extra_params 行为不一致；生成 worker 同样传入冻结额外参数。
- 本地 conda 启动统一管理 API 与 runner，默认 Docker 不启动 worker；文档和配置示例明确端点注册、凭证和本地模型条件。
- 新增解析 seed、本地模型、参数清空、试跑 URL/参数、设置回读、端点缺失等回归。前端 120 项、TypeScript、ESLint 通过；后端无 conftest 单元 7 项、Ruff 和 OpenAPI 漂移检查通过。工作树依赖 junction 导致 Turbopack 拒绝构建，最终主工作树再运行默认 build。

---

## 用户工作流全面 Review（2026-09-12）

- 基于 `03ebbdd` 从安装配置、认证权限、解析清洗、分块与任务恢复、生成审核到编组导出完成评审。
- 新增报告 `docs/reviews/2026-09-12-user-workflow-review.md`，记录 21 项业务问题（16 项 P1、5 项 P2）及一项 Windows 质量检查脚本问题；逐项提供触发步骤、证据边界、修复方案和回归验收条件。
- 定向执行真实 React 组件/hook、业务函数及 ORM 时序复现；外部数据库/HTTP 使用隔离测试设施或 fake，不调用真实付费模型服务。
- 前端 ESLint、TypeScript、95 项 Vitest、production build、API 合同漂移检查通过。后端 Ruff、422 项 pytest 全部通过，覆盖率 65.13%；带日志重定向的前端门禁脚本警告中断问题已单独记录。

---

## 用户流程审查修复第四批：任务恢复与执行策略（2026-09-12）

- 对应 R09–R11、R21 后端：新增可重建的 handler 生命周期注册表，取消、失败、租约回收和人工重试在同一事务内同步 Task 与 ParseJob、CleaningJob、ChunkSet、生成批次及导出对象。
- ParseJob、CleaningJob 记录关联 Task、取消状态和稳定错误信息；失败或取消的清洗任务不再复用空工作台，人工 retry 原子重绑业务对象。
- runner 改为单执行槽、独立 reaper 和独立心跳；过期但尚未记录业务错误的任务按临时基础设施中断回队，旧 token 不可续租或发布。
- TaskPolicy 按项目和任务类型冻结重试次数、超时和快照；0 次重试表示一次执行。领取时通过数据库锁和活跃任务计数执行跨 runner 并发上限，批量父任务与子任务分别使用对应策略。
- 生成 retry 创建新的 Batch/Run/Task 并继承源任务的冻结策略；父聚合等待所有子任务终态后写入完整摘要。补齐满额队列不饿死其他类型、取消锁序和默认策略并发切换保护。
- 新增 r04 可恢复任务策略迁移，含状态枚举、业务绑定、策略约束及同类型唯一默认策略。新增和修订任务恢复、迁移、策略、并发、取消、迟到发布和 LLM 分类回归。
...
## 2026-09-12 · 用户流程审查修复第二批：清洗编辑与保存（R05–R08）

- 重构清洗会话状态：自动保存、手动保存、保存并继续及提交审核复用同一串行保存队列；成功返回具体 revision，失败按冲突、租约失效、请求失败及章节切换返回判别结果。保存响应只推进已发送正文的基线，保存期间的新增输入保持未保存并继续排队。
- 提交审核前立即停止防抖并等待最新正文保存，携带返回的新 revision 提交；同步阻止重复提交及提交期间编辑、切换。章节切换、卸载和迟到响应均以会话身份保护。
- 租约 ID 驱动心跳生命周期，异步 acquire 后每 30 秒续租；租约丢失保留草稿并提供重新获取、复制入口。重新获取时复核远端 revision，发生变化时保留草稿并暂停覆盖；只有显式点击重新载入才丢弃草稿。
- 页面仅在保存成功后继续导航或切换，失败保留原章节、草稿和确认框；返回文档也经过草稿保护。移除章节切换时页面与 hook 重复发起的详情和 acquire 请求，取消冲突对话框 Escape 隐式重新载入行为。
